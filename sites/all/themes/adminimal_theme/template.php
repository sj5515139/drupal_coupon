...
function adminimal_preprocess_page(&$vars) {
	...
	foreach($query as $q){
		...
		foreach($scid as $sc){
			$subcat= clean($sc->url_alias);
		}
		$loc_cat[] = 'location/'.$loc.'/'.$subcat;
	}
	...
	foreach($query as $q){
		...
		foreach($sid as $st){
			$str= clean($st->url_alias);
		}
		$loc_str[] = 'location/'.$loc.'/'.$str;
	}
	
	$coupons = array();
	$query = db_query("SELECT * FROM coupan");
	foreach($query as $q){
		$coupons[] = 'coupon/'.$q->coupan_id;
	}
	
	if($path == 'home' || $path == 'category' || $path == 'store' || $path == 'location' || $path == 'bank' || in_array($path, $category) 
	|| in_array($path, $sub_cat) || in_array($path, $store) || in_array($path, $location) || in_array($path, $bank) 
	|| in_array($path, $loc_str) || in_array($path, $loc_cat) || in_array($path, $coupons) || preg_match('/search/',$path)){
		$css_path = drupal_get_path('theme', 'adminimal');
		drupal_add_css($css_path . '/css/homepage_style.css', array('group' => CSS_THEME, 'weight' => 900, 'preprocess' => TRUE));
		$vars['theme_hook_suggestion'] = 'page__homepage';
	}
...
function adminimal_form_alter(&$form, &$form_state, $form_id){
  if($form_id == "simplenews_block_form_1"){
        $form['mail']['#attributes'] = array('placeholder' => t('Drop Your Email'));
